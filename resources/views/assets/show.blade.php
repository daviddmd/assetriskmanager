                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="description" id="description"
                                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" disabled>{{$threat->description}}</textarea>
                    </div>
                    <!--Mudar para Tabela-->
                    @if($threat->controls->count()>0)
                        <div class="mb-6">
                            <label for="controls"
                                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{__("Controls")}}</label>
                            <div id="controls">
                                <ul class="list-disc">
                                    @foreach($threat->controls as $control)
                                        <li>
                                            <a href="{{route("controls.edit",$control->id)}}"
                                               target="_blank">{{$control->name}}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
